...
//     }
    ...
    public: inline bool trim_leaf(const unsigned int v) {
        ...
        remove_edge(v,p);
        make_phylogeny(p); //RUCHI 30th May
        return true;
    }

    //take care of node v
    //v may have degree 2 or 1
    public: inline void make_phylogeny(const unsigned int v) {
        if((degree(v)==2) && !(root == v)) {
            std::vector<unsigned int> adjs;
            BOOST_FOREACH(const unsigned int &i, adjacent(v)) {
                adjs.push_back(i);
                remove_edge(i,v);
            }
            if(adjs.size()!=2)  ERROR_exit("ERROR");
            add_edge(adjs[0],adjs[1]);
            return;
        }
        if(degree(v)==1) {
            if(root!=v) ERROR_exit("ERROR");
            if (degree(root) != 1) ERROR_exit("ERROR");
            unsigned int c = *adjacent(root).begin();
            remove_edge(c,root);
            root = c;
            return;
        }
...
//            if(u_adj==NONODE) {
//                if(b1==u_leaf) m = b2; else if(b2 == u_leaf) m = b1;
//                //MSG("SPecial case in SPR_TO_EDGE!!!!");
//            }
//            BOOST_FOREACH(const unsigned int &v, ch)
//                if(v==m) u_adj = m;
    ...
    public: inline void rootInit(){
        if(root!=0) {
            unsigned int old_root = root;
            //std::cout<<"Root "<<old_root;
            ...
            BOOST_FOREACH(const unsigned int &i, adjacent_vector(0)) {
                //std::cout<<"\n i "<<i;
                remove_edge(i,0);
                //std::cout<<" rem edge 0 n "<<i;
                adj_zero.push_back(i);  }

            BOOST_FOREACH(const unsigned int &i, adjacent_vector(old_root)) {
                //std::cout<<"\n i "<<i;
                remove_edge(i,old_root);
                //std::cout<<" rem edge old_root n "<<old_root<<i;
                add_edge(i,0);
                //std::cout<<" add edge 0 and "<<i;
            }

            for(unsigned int s=0,sEE=adj_zero.size(); s<sEE; ++s) {
                //std::cout<<"\n add edge "<<adj_zero[s]<<old_root;
