...
template<class TREE>
inline unsigned int compute_rf_score(TREE &s_tree, TREE &g_tree, LCAmapping &s_map, std::pair<unsigned int,unsigned int> &node_count,unsigned int s_int) {
    ...
    //MSG("Node count:"<<node_count.first<<" "<<s_int);
    

    //std::cout<<" s_int "<<s_int;
    //std::cout<<" node_count.first "<<node_count.first;
    //std::cout<<" score "<<score;
    score =  score + s_int - node_count.first;
            //score  - node_count.first + node_count.second - 2;
    //MSG("score is:"<<score);
    return score;
}
